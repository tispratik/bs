...
module PieChart
  ...
  class MotionChart
    ...
    def header
      content_tag(:span, "", :id => name, :style => "width: #{options[:width]}px; height: #{options[:height]}px;")
    end
  ...
  class Chart
    ...
    def initialize(view_instance, dates, options={}, *args)
      @helpers = view_instance
      @dates = dates
      @options = options.reverse_merge({:width => 400, :height => 220})
      @lines = []
    ...
    def render_head
      "<span class=\"chartPanel\">\n"+
        "<span class=\"dashChart\" id=\"#{@name}\" style=\"width: #{@options[:width]}px; height: #{@options[:height]}px;\"></span>\n" +
      "</span>\n" +
    ...
    def render_foot
      "var #{@name} = new google.visualization.#{@google_chart_name}(document.getElementById('#{@name}'));\n" +
        "#{@name}.draw(#{@name}_data, {#{ruby_to_javascript_options(@options)}});\n</script>"
    end


      # converts {:my_title=>'title', :my_width=>100} to "myTitle: 'title', myWidth: 100"
    def ruby_to_javascript_options(options)
      result = []
  ...
  module Helpers
    ...
    def chart_for(google_chart_name, dates, options={}, *args, &block)
      ...
      yield chart
      #concat(chart.render, block.binding)  # rails 2.1.2
     concat(chart.render)  # rails 2.3.x
    end
 end

  ActionView::Base.send :include , PieChart::Helpers
